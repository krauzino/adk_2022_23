...
double Algorithms::computeAspect(const QPoint3D &p1, const QPoint3D &p2,const QPoint3D &p3)
{
    // Compute aspects of triangle
    double ux = p2.x() - p1.x();
    double uy = p2.y() - p1.y();
    double uz = p2.getZ() - p1.getZ();
    ...
    // Direction of the aspects
    return atan2(nx,ny);
}


std::vector<Triangle>Algorithms::analyzeAspect(const std::vector<Edge> &dt)
{
    // Analyze Aspect
    std::vector<Triangle> triangles;
...
{
    //Transform polygon coor bon MinMaxBox
